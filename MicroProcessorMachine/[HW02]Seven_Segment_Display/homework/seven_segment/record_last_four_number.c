...
const bool IS_USEED[] = {
  ...
  false, true, false, false,  // X 0 L R
...
void loop() {
  ...
  if (keyScan() == true) {           //在讀取按鈕時 當按鈕被按下 會讀到的數值會是0 (原因:看電路圖 當按鈕按下時電路將導通接地 電位會輸出0)
    ...
    if (seg_index == 14) {  //按鈕為功能:左到右飛入
      for (int i = (RECORD_SIZE - 1); i >= 0; i--) {
        for (int pos = 0; pos < RECORD_SIZE; pos++) {
          delay(DELAY_TIME);
          if (pos != 0) {
            WriteNumberToSegment(pos - 1, 3);
          }
          WriteNumberToSegment(pos, segment_map_index[i]);
        }
        delay(DELAY_TIME);
    ...
    } else if (IS_USEED[seg_index]) {       //按鈕為數字
      //紀錄最後一個數字
      for (int i = 0; i < (RECORD_SIZE - 1); i++) {
        segment_map_index[i] = segment_map_index[i + 1];
      }
      segment_map_index[3] = seg_index;
      delay(DELAY_TIME);
    }
  }
  //顯示數值
