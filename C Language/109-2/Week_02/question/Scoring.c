...
double get_average(double total, int num) {
    ...
    printf("total/num=%lf\n", d_temp);
    printf("d_temp*100=%.lf\n", d_temp * 100.0);
    int i_temp = (int)(d_temp * 100.0);
    printf("(int)(d_temp*100)=%d\n", i_temp);
    printf("(double)i_temp/100=%lf", (double)i_temp / 100);
...
int main() {
    ...
    while (1) {
        ...
        flag = true;
        printf("%.2lf\n", get_average(total_scores[3] * 0.1, std_num));
    }
    return 0;
}